...
import { Trophy, Target, BookOpen, Swords, TrendingUp, Award } from 'lucide-react';
        ...
        <div className="space-y-6">
          ...
          >
            ...
            <div className="bg-slate-900/50 rounded-lg p-4 mb-4 border border-blue-500/30">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-blue-200 font-gaming">NIVEL 1</span>
                <span className="text-sm text-blue-200">0 / 100 XP</span>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Monedas', value: '0', icon: '🪙', color: 'yellow' },
                { label: 'Gemas', value: '0', icon: '💎', color: 'cyan' },
                { label: 'Victorias', value: '0', icon: '🏆', color: 'green' },
                { label: 'Racha', value: '0', icon: '🔥', color: 'orange' },
              ].map((stat, i) => (
                <div key={i} className="bg-slate-900/50 rounded-lg p-3 border border-blue-500/30 text-center">
                  <div className="text-2xl mb-1">{stat.icon}</div>
                  <p className={`text-2xl font-bold font-gaming text-${stat.color}-400`}>{stat.value}</p>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              {[
                ...
                { title: 'Práctica', icon: Target, color: 'purple', gradient: 'from-purple-500 to-pink-600', bg: 'bg-purple-950', onClick: () => {} },
            ...
            <Card className="bg-slate-800 border-2 border-indigo-500">
              ...
              <CardContent className="pt-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  {[
                    { label: 'Batallas', value: '0', icon: Swords, color: 'green' },
                    { label: 'Logros', value: '0', icon: Award, color: 'purple' },
                    { label: 'Precisión', value: '0%', icon: Target, color: 'blue' },
                    { label: 'Trofeos', value: '0', icon: Trophy, color: 'yellow' },
                  ].map((stat, i) => (
                    <div key={i} className="text-center">
                      ...
                      <p className={`text-3xl font-bold font-gaming text-${stat.color}-400 mb-1`}>{stat.value}</p>
